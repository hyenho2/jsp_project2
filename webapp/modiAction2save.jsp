...
<body>
	...
	<%
		...
		if (userID == null) {
			...
			if (bbs.getBbsTitle() == null || bbs.getBbsContent() == null || bbs.getBbsNick() == null) {
				PrintWriter script = response.getWriter();
				script.println("<script>");
				...
				int result = BbsDAO.write(bbs.getBbsTitle(), userID, bbs.getBbsContent(), bbs.getBbsNick());
				if (result == -1) {
					PrintWriter script = response.getWriter();
					script.println("<script>");
					script.println("alert('글 수정을 실패했습니다')");
					script.println("history.back()");
					script.println("</script>");
				} else {
					PrintWriter script = response.getWriter();
